...
template <size_t N> class superlong
{
    ...
    public:
        superlong() : value({0}) {}
    superlong(const array<uint32_t, N / 32 + (N % 32 != 0)>&
    v):value(v) {}
    superlong(const string & s);
    superlong<N> &operator=(const superlong<N>& s);
    superlong<N> operator+(const superlong<N>& s)const;
    template <size_t X> friend ostream& operator<< (ostream&
    stream, superlong<X> &s);
    };
    ...
    if(!check)throw error("invalid_argument");
    }
    value = {0};
    int i = 0;
    auto ptr_2 = s.end();
    ...
    while (ptr_b != ptr_2) {
    auto ptr_1 = ptr_2 - 8;
    if (ptr_1 < ptr_b)ptr_1 = ptr_b;
    value[i] = stoul(string(ptr_1,ptr_2),0,16);
    ptr_2 = ptr_1;
    i++;
    }
    }
    template <size_t N> superlong<N> &superlong<N> ::
    operator=(const
    superlong<N> &s)
    {
    value = s.value;
    ...
    template <size_t N>string superlong<N>::toHex()
    {
    string s;
    for (auto e:value)
